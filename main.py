...
class Clicker:
    def __init__(self, parent):
        ...
        self.gear['auto clicker'] = Gear('auto clicker', 15, per_second=1)
        self.gear['clicker'] = Gear('clicker', 10, quantity=1)
        self.purchase_buttons['clicker'] = tk.Button(parent, text='Extra click. Initial cost is %d: 1' % self.gear['clicker'].cost,
            command=lambda: self.purchase('clicker'))
        self.purchase_buttons['auto clicker'] = tk.Button(parent, text='Auto clicker. Initial cost is %d: 0' % self.gear['auto clicker'].cost,
    ...
    def purchase(self, name):
        if self.current_clicks >= self.gear[name].cost:
            ...
            self.current_click_label.config(text='%d' % self.current_clicks)
            self.purchase_buttons[name].config(text=self.purchase_buttons[name]['text'].split(': ')[0] + ': {:.1f}: {} clicks'.format(self.gear[name].cost,
